...
typedef struct __attribute__((__packed__)) {
    ...
    uint8_t next;  // next page containing the next 64K of the file; 0 if no next page (Because allocation always starts
                   // at 0 and pages can't refer to a lower page, 0 can never occur normally)
...
static AppfsMeta const* appfsMeta = NULL;  // mmap'ed flash
#ifndef BOOTLOADER_BUILD
static esp_partition_t const* appfsPart = NULL;
    ...
    for (int sec = 0; sec < APPFS_META_CNT; sec++) {
        ...
        if (memcmp(hdr.magic, APPFS_MAGIC, 8) == 0) {
            ...
            if (crc == expectedCrc) {
                ...
                ESP_LOGD(TAG, "Meta sector %d does not have a valid CRC (have %" PRIX32 " expected %" PRIX32 ".", sec,
                         crc, expectedCrc);
            }
        } else {
...
IRAM_ATTR
#endif
static int appfsGetFirstPageFor(char const* filename) {
    for (int j = 0; j < APPFS_PAGES; j++) {
        if (appfsMeta[appfsActiveMeta].page[j].used == APPFS_USE_DATA &&
            strcmp(appfsMeta[appfsActiveMeta].page[j].name, filename) == 0) {
            return j;
        }
    }
    // Nothing found.
...
int appfsExists(char const* filename) {
    return (appfsGetFirstPageFor(filename) == -1) ? 0 : 1;
}

appfs_handle_t appfsOpen(char const* filename) {
...
void appfsEntryInfo(appfs_handle_t fd, char const** name, int* size) {
    if (name) *name = appfsMeta[appfsActiveMeta].page[fd].name;
    if (size) *size = appfsMeta[appfsActiveMeta].page[fd].size;
}

void appfsEntryInfoExt(appfs_handle_t fd, char const** name, char const** title, uint16_t* version, int* size) {
    if (name) *name = appfsMeta[appfsActiveMeta].page[fd].name;
    if (title) {
...
appfs_handle_t appfsNextEntry(appfs_handle_t fd) {
    ...
    while (appfsMeta[appfsActiveMeta].page[fd].used != APPFS_USE_DATA ||
           appfsMeta[appfsActiveMeta].page[fd].name[0] == 0xff) {
        fd++;
...
size_t appfsGetTotalMem() {
    ...
    for (int i = 0; i < APPFS_PAGES; i++) {
        if ((appfsMeta[appfsActiveMeta].page[i].used == APPFS_USE_FREE ||
             appfsMeta[appfsActiveMeta].page[i].used == APPFS_USE_DATA) &&
            page_in_part(i)) {
            ret += APPFS_SECTOR_SZ;
        }
    }
...
IRAM_ATTR __attribute__((weak)) void appfs_bootloader_munmap(void const* mapping) {
    bootloader_munmap(mapping);
}

IRAM_ATTR __attribute__((weak)) esp_err_t appfs_bootloader_flash_read(size_t src_addr, void* dest, size_t size,
                                                                      bool allow_decrypt) {
...
#if CONFIG_IDF_TARGET_ESP32
    DPORT_REG_CLR_BIT(DPORT_PRO_CACHE_CTRL1_REG, (DPORT_PRO_CACHE_MASK_IRAM0) | (DPORT_PRO_CACHE_MASK_IRAM1 & 0) |
                                                     (DPORT_PRO_CACHE_MASK_IROM0 & 0) | DPORT_PRO_CACHE_MASK_DROM0 |
                                                     DPORT_PRO_CACHE_MASK_DRAM1);
    DPORT_REG_CLR_BIT(DPORT_APP_CACHE_CTRL1_REG, (DPORT_APP_CACHE_MASK_IRAM0) | (DPORT_APP_CACHE_MASK_IRAM1 & 0) |
...
IRAM_ATTR esp_err_t appfs_bootloader_read(int fd, size_t src_addr, void* dest, size_t size) {
    ...
    for (int i = 0; i < 255; i++) {
        if (pos + APPFS_SECTOR_SZ - 1 >= src_addr) {
            ...
            esp_err_t r = appfs_bootloader_flash_read(appfsPartOffset + ((page + 1) * APPFS_SECTOR_SZ) + offset_in_page,
                                                      destp + have_read, rsize, true);
...
esp_err_t appfsInit(int type, int subtype) {
    ...
    r = esp_partition_mmap(appfsPart, 0, APPFS_SECTOR_SZ, SPI_FLASH_MMAP_DATA, (void const**)&appfsMeta,
                           &appfsMetaMmapHandle);
    if (r != ESP_OK) return r;
    if (findActiveMeta() != ESP_OK) {
        // No valid metadata half-sector found. Initialize the first sector.
...
static esp_err_t writePageInfo(int newMeta, int page, AppfsPageInfo* pi) {
    return esp_partition_write(appfsPart, newMeta * APPFS_META_SZ + (page + 1) * APPFS_META_DESC_SZ, pi,
                               sizeof(AppfsPageInfo));
}

// This essentially writes a new meta page without any references to the file indicated.
esp_err_t appfsDeleteFile(char const* filename) {
    esp_err_t r;
    int next = -1;
    int newMeta;
    AppfsHeader hdr;
...
esp_err_t appfsRename(char const* from, char const* to) {
    esp_err_t r;
    int newMeta;
    AppfsHeader hdr;
    AppfsPageInfo pi;
...
esp_err_t appfsCreateFileExt(char const* filename, char const* title, uint16_t version, size_t size,
...
esp_err_t appfsCreateFile(char const* filename, size_t size, appfs_handle_t* handle) {
    return appfsCreateFileExt(filename, filename, 0, size, handle);
}

esp_err_t appfsMmap(appfs_handle_t fd, size_t offset, size_t len, void const** out_ptr, spi_flash_mmap_memory_t memory,
    ...
    if (appfsMeta[appfsActiveMeta].page[page].size < (offset + len)) {
        ESP_LOGD(TAG, "Can't map file: trying to map byte %zu in file of len %" PRId32, (offset + len),
                 appfsMeta[appfsActiveMeta].page[page].size);
...
IRAM_ATTR esp_err_t appfsMmapAt(appfs_handle_t fd, size_t offset, size_t len, size_t mmu_vaddr,
    ...
    if (offset + len > appfsMeta[appfsActiveMeta].page[fd].size) {
        ESP_LOGD(TAG, "Can't map file: trying to map byte %zu in file of len %" PRId32, (offset + len),
                 appfsMeta[appfsActiveMeta].page[fd].size);
        return ESP_ERR_INVALID_SIZE;
    }
...
void appfsDump() {
    ...
    printf("\n");
    for (int i = 0; i < APPFS_PAGES; i++) {
        if (appfsMeta[appfsActiveMeta].page[i].used == APPFS_USE_DATA &&
            appfsMeta[appfsActiveMeta].page[i].name[0] != 0xff) {
